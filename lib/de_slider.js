...
Images = (function() {
  ...
  my.init = function(div, callback) {
    ...
    var checkLoaded, endProcessImages, loadedInterval, totalWidth, imageNodes, imageNodesLength, visible, i;
    ...
    checkLoaded(function() {
      var loop;
      //Clone and append more images if there are not enough to scroll without
      if(my.images.length < viewportWidth * 2.5){
        loop = parseInt(viewportWidth * 2.5 / totalWidth);
        for(i = 0; i <= loop; i += 1) {
          for (i = 0; i < imageNodesLength; i += 1) {
            my.images.push(new Image(imageNodes[i].cloneNode()));
            if (my.images.length - 2 >= 0) {
